...
export const DEFAULT_SUGGESTION_MESSAGE_FORMAT = `Bạn là chuyên gia về tán tỉnh và tâm lý phụ nữ. Nhiệm vụ của bạn là tư vấn cho người dùng về tâm lý của đối phương trong câu chuyện dựa trên ngữ cảnh được chọn, cách đối phương nhìn nhận người dùng dựa trên ngữ cảnh, và cách đạt được mục tiêu hẹn hò mà người dùng đề ra. Tính cách của bạn là thẳng thắn, hài hước, và hơi châm biếm. Bạn sẽ trả lời ngắn gọn, đi thẳng vào vấn đề, không đi vào chi tiết quá nhiều trừ khi được yêu cầu, và thành thật với người dùng về tình hình thực tế thay vì nịnh nọt cảm xúc của họ.
---
{{conversationHistory}}
---
...
export async function createConsultationPrompt_EN(
  ...
  let messageFormat = DEFAULT_SUGGESTION_MESSAGE_FORMAT;
  try {
    const kvFormat = await kv.get(KV_KEYS.suggestionMessageFormat);
  ...
  let selectedMessageInstruction = "";
  if (selectedMessage) {
    selectedMessageInstruction = `User has selected this message: ${selectedMessage.text}`;
  } else {
    selectedMessageInstruction = ``;
  }

  let questionInstruction = "";
  if (question) {
    questionInstruction = `User question is: ${question}`;
  }

  // Replace placeholders in format
  const prompt = messageFormat
    .replace("{{conversationHistory}}", conversationHistory)
    .replace("{{selectedMessageInstruction}}", selectedMessageInstruction)
    .replace("{{questionInstruction}}", questionInstruction);
...
export const DEFAULT_ANALYZE_INTENT_MESSAGE_FORMAT = `You are an expert at analyzing romantic conversations and reading emotional signals. Your task is to analyze a message and provide insights about:
1. Interest level - how interested the sender appears to be (0-100 score)
2. Emotional tone - what emotions are being expressed
3. State read - a brief interpretation of what's happening
4. Recommended response directions - strategic ways to reply
...
export async function createAnalyzeIntentPrompt(context, message) {
  ...
  let messageFormat = DEFAULT_ANALYZE_INTENT_MESSAGE_FORMAT;
  try {
    const kvFormat = await kv.get(KV_KEYS.analyzeIntentMessageFormat);
    if (kvFormat) messageFormat = kvFormat;
  } catch (error) {
    console.error(
      "Failed to fetch ANALYZE_INTENT_MESSAGE_FORMAT from KV:",
      error,
    );
  }
...
export const DEFAULT_GENERATE_FROM_DIRECTION_MESSAGE_FORMAT = `You are an expert at crafting engaging, emotionally resonant messages in romantic conversations. Your task is to generate a response that follows a specific strategic direction.
...
- Tone: {{directionTone}}
- Emoji style: {{directionEmoji}}
- Approach: {{directionDescription}}
...
export async function createGenerateFromDirectionPrompt(
  context,
  messageText,
  direction,
) {
  ...
  let messageFormat = DEFAULT_GENERATE_FROM_DIRECTION_MESSAGE_FORMAT;
  try {
    const kvFormat = await kv.get(KV_KEYS.generateFromDirectionMessageFormat);
    if (kvFormat) messageFormat = kvFormat;
  } catch (error) {
    console.error(
      "Failed to fetch GENERATE_FROM_DIRECTION_MESSAGE_FORMAT from KV:",
      error,
    );
  }
  ...
  const prompt = messageFormat
    ...
    .replace("{{directionLabel}}", direction.label)
    .replace("{{directionTone}}", direction.tone)
    .replace("{{directionEmoji}}", direction.emoji)
    .replace("{{directionDescription}}", direction.description);
...
export const DEFAULT_GRADE_OWN_MESSAGE_MESSAGE_FORMAT = `You are an expert at evaluating messages in romantic conversations. Your task is to grade a user's message and provide constructive feedback.
...
export async function createGradeOwnMessagePrompt(context, message) {
  ...
  let messageFormat = DEFAULT_GRADE_OWN_MESSAGE_MESSAGE_FORMAT;
  try {
    const kvFormat = await kv.get(KV_KEYS.gradeOwnMessageMessageFormat);
    if (kvFormat) messageFormat = kvFormat;
  } catch (error) {
    console.error(
      "Failed to fetch GRADE_OWN_MESSAGE_MESSAGE_FORMAT from KV:",
      error,
    );
  }
